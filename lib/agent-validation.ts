...
type CitationEvidence = { products?: Array<{ id: string }>; series: Array<{ evidenceId: string }>; competitors: Array<{ evidenceId: string }>; competitorTrends?: Array<{ evidenceId: string }>; crossSignals?: Array<{ evidenceId: string }>; healthScores?: Array<{ evidenceId: string }>; goals?: Array<{ evidenceId: string }>; missions?: Array<{ evidenceId: string }>; truncated?: { metrics?: boolean; competitors?: boolean } };
...
export function validateAgentCitations<T extends { findings: CitationFinding[] }>(result: T, evidence: CitationEvidence): T {
  ...
  if ((evidence.truncated?.metrics || evidence.truncated?.competitors) && !/(?:\bincomplete\b|\bpartial\b|\btruncated\b|不完整|部分数据|已截断)/i.test(result.findings.map((finding) => `${finding.title} ${finding.detail || ''}`).join(' '))) throw new Error('Truncated evidence must be disclosed as incomplete in the Agent output.');
  return result;
}
