...
export const agentDefinitions = {
  ...
  operations_analyst: { name: 'Operations Analyst', dashboard: 'cloudflare_operations', focus: 'Detect customer-facing request, error, latency, CPU, queue, database, object-storage, backend-health, and delivery regressions. Prioritize measurable user impact and reversible checks; treat deployment signals as reliability context rather than business outcomes.', metrics: ['requests', 'errors', 'error_rate', 'subrequests', 'wall_time', 'cpu_time_p50', 'cpu_time_p99', 'd1_reads', 'd1_writes', 'r2_requests', 'r2_errors', 'r2_payload_bytes', 'r2_metadata_bytes', 'r2_objects', 'r2_pending_uploads'] },
...
export const deliveryHealthMetrics = ['vercel_failed_deployments', 'vercel_last_completed_deployment_success', 'pages_failed_deployments', 'pages_last_completed_deployment_success', 'supabase_project_healthy'] as const;
export function agentAllowedMetrics(preset: AgentPreset): readonly string[] { return preset === 'operations_analyst' ? [...agentDefinitions[preset].metrics, ...queueOperationMetrics, ...deliveryHealthMetrics] : agentDefinitions[preset].metrics; }
