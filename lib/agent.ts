...
export async function invokeAgentProvider(workspaceId: string, provider: AgentProvider, question: string, preset: AgentPreset, evidence: Awaited<ReturnType<typeof buildEvidence>> & Record<string, unknown> & { images?: ReturnType<typeof agentImageEvidence> }, evidenceSkills: EvidenceSkill[], abortSignal?: AbortSignal, images: AgentImageInput[] = []) {
  ...
  try {
    result = await invokeOpenAiCompatibleWithFallback({ baseUrl: baseURL, apiKey, model, system, prompt, images, preferredProfile: compatibility.profile, allowFallback: !compatibility.validatedAt, abortSignal });
  } catch (error) {
  ...
  try { findings = validateOutput(result, 'initial'); }
    ...
    try {
      repaired = await invokeOpenAiCompatibleWithFallback({ baseUrl: baseURL, apiKey, model, system: repair.system, prompt: repair.prompt, images, preferredProfile: compatibility.profile, allowFallback: !compatibility.validatedAt, abortSignal });
    } catch (error) {
