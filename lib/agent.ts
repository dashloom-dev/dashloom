...
import { buildSeoOpportunities, SEO_ANALYSIS_POLICY } from './agent-seo';
...
import { invokeOpenAiCompatibleWithFallback, OpenAiCompatibleOutputError, OpenAiCompatibleRequestError, parseProviderCompatibility } from './openai-compatible';
import { agentImageEvidence, AgentVisionUnsupportedError, type AgentImageInput } from './agent-images';
...
const reasoningSummaryStepSchema = z.object({
  title: z.string().max(160),
  detail: z.string().max(700),
  evidenceRefs: z.array(z.string().max(160)).min(1).max(8),
});

const executionTraceStepSchema = z.object({
...
export const agentResultSchema = z.preprocess(normalizeAgentResultInput, z.object({
  summary: z.string().max(1200),
  reasoningSummary: z.array(reasoningSummaryStepSchema).min(1).max(4).optional(),
  executionTrace: z.array(executionTraceStepSchema).max(12).optional(),
...
export async function invokeAgentProvider(workspaceId: string, provider: AgentProvider, question: string, preset: AgentPreset, evidence: Awaited<ReturnType<typeof buildEvidence>> & Record<string, unknown> & { images?: ReturnType<typeof agentImageEvidence> }, evidenceSkills: EvidenceSkill[], abortSignal?: AbortSignal, images: AgentImageInput[] = [], investigation?: { onEvidenceUpdated: () => Promise<void>; onTextDelta?: (text: string) => void; onTextReset?: () => void }) {
  ...
  try {
    result = await invokeOpenAiCompatibleWithFallback({ baseUrl: baseURL, apiKey, model, system, prompt, images, preferredProfile: compatibility.profile, allowFallback: !compatibility.validatedAt, abortSignal, onTextDelta: investigation?.onTextDelta });
  } catch (error) {
    if (images.length && error instanceof OpenAiCompatibleRequestError && [400, 415, 422].includes(error.statusCode)) throw new AgentVisionUnsupportedError(error.statusCode);
  ...
  let inputTokens = result.usage.inputTokens || Math.max(1, Math.ceil((system.length + prompt.length) / 4) + imageTokenAllowance);
  ...
  try { findings = validateOutput(result, 'initial'); }
    ...
    try {
      repaired = await invokeOpenAiCompatibleWithFallback({ baseUrl: baseURL, apiKey, model, system: repair.system, prompt: repair.prompt, images, preferredProfile: compatibility.profile, allowFallback: !compatibility.validatedAt, abortSignal, onTextDelta: investigation?.onTextDelta });
    } catch (error) {
    ...
    const repairInputTokens = repaired.usage.inputTokens || Math.max(1, Math.ceil((repair.system.length + repair.prompt.length) / 4) + imageTokenAllowance);
...
export async function runWorkspaceAgent(workspaceId: string, question: string, preset: AgentPreset = 'portfolio_analyst', trigger: AnalysisTrigger = 'chat', conversationId?: string | null, scope: AgentProductScope = { mode: 'workspace', productId: null }, options: AgentRunOptions = {}) {
  const db = getDb();
  const executionTrace: AgentRunProgress[] = [];
  const emitProgress = async (progress: Omit<AgentRunProgress, 'startedAt' | 'completedAt'>) => {
    const now = new Date().toISOString();
    const existingIndex = executionTrace.findIndex((item) => item.stage === progress.stage);
    const existing = existingIndex >= 0 ? executionTrace[existingIndex] : null;
    const next: AgentRunProgress = {
      ...progress,
      startedAt: existing?.startedAt || now,
      completedAt: progress.status === 'in_progress' ? null : now,
    };
    if (existingIndex >= 0) executionTrace[existingIndex] = next;
    else executionTrace.push(next);
    await options.onProgress?.(next);
  };
  await emitProgress({ stage: 'preparing', label: 'Preparing the run', detail: 'Checking the selected specialist, scope, and provider configuration.', status: 'in_progress' });
  ...
  await emitProgress({ stage: 'preparing', label: 'Run prepared', detail: `${definition.name}, the requested scope, and the configured provider passed readiness checks.`, status: 'completed' });
  if (options.images?.length) await emitProgress({ stage: 'images_validated', label: 'Images validated', detail: `${options.images.length} image${options.images.length === 1 ? '' : 's'} passed type, size, and file-signature checks and will be sent to the configured model.`, status: 'completed' });
  await emitProgress({ stage: 'evidence_frozen', label: 'Freezing evidence', detail: 'Collecting the current bounded evidence and conversation context for this run.', status: 'in_progress' });
  ...
  try {
    ...
    const result = await invokeAgentProvider(workspaceId, provider, question, preset, evidence, evidenceSkills, options.abortSignal, options.images, { onTextDelta: options.onTextDelta, onTextReset: options.onTextReset, onEvidenceUpdated: async () => {
    ...
    await db.update(analysisRuns).set({ status: 'success', findingsJson: JSON.stringify({ ...findings, executionTrace }), inputTokens: result.inputTokens, outputTokens: result.outputTokens, finishedAt }).where(eq(analysisRuns.id, runId));
    ...
    await emitProgress({ stage: 'persisting', label: 'Verified result saved', detail: 'The answer, usage record, suggested actions, and evidence snapshot were written successfully.', status: 'completed' });
    await emitProgress({ stage: 'completed', label: 'Run completed', detail: `${result.inputTokens + result.outputTokens} model tokens were recorded with the evidence snapshot.`, status: 'completed' });
    const storedFindings: AgentResult = { ...findings, executionTrace: [...executionTrace] };
    await db.update(analysisRuns).set({ findingsJson: JSON.stringify(storedFindings) }).where(eq(analysisRuns.id, runId));
    return { runId, preset, agent: definition.name, evidence, findings: storedFindings };
  } catch (error) {
    const failure = classifyAgentFailure(error);
