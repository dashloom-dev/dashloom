...
async function currentHealth() { const checkedAt = new Date().toISOString(); try { await getDb().run(sql`select 1`); return { level: 'operational', database: 'reachable', checkedAt } as const; } catch { return { level: 'degraded', database: 'unreachable', checkedAt } as const; } }
