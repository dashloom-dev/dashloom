...
export async function POST(request: Request) {
  ...
  try { const outcome = await syncGoogleWorkspace(workspace.id); return NextResponse.json({ ...outcome, calculated: await refreshCalculatedMetricsSafely(workspace.id) }); }
  catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : 'Google sync failed' }, { status: 422 }); }
}
