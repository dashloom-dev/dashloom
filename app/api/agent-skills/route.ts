...
export async function DELETE(request: Request) { const value = await context(request); if (!value) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 }); if (!['owner', 'admin'].includes(value.workspace.role)) return NextResponse.json({ error: 'Owner or admin access required' }, { status: 403 }); const id = new URL(request.url).searchParams.get('id'); if (!id) return NextResponse.json({ error: 'Skill id is required' }, { status: 400 }); const result = await getDb().update(agentSkillManifests).set({ enabled: false, updatedAt: new Date().toISOString() }).where(and(eq(agentSkillManifests.id, id), eq(agentSkillManifests.workspaceId, value.workspace.id))).returning({ id: agentSkillManifests.id }); if (!result.length) return NextResponse.json({ error: 'Skill not found' }, { status: 404 }); return NextResponse.json({ disabled: true }); }
