...
export default function ChineseHome() {
  return <main lang="zh-CN">
    ...
    <section className="value-section" id="why"><div className="section-intro"><span>为什么选择 DASHLOOM</span><h2>不只是查看数据，<br />还要理解变化。</h2></div><div className="value-grid"><article><b>01</b><h3>基于证据回答</h3><p>重要结论关联到具体产品、指标、时间范围、来源和数据新鲜度，而不是凭空生成答案。</p></article><article><b>02</b><h3>自动生成定期简报</h3><p>Agent 持续监控数据变化，并生成每日、每周和每月报告，推送到关联渠道。</p></article><article><b>03</b><h3>自由选择 AI 用法</h3><p>开源版使用自己的 OpenAI 兼容 API；云版本根据套餐提供托管 AI 额度和可靠交付。</p></article></div></section>
    <section className="open-section" id="open-source"><div><span className="eyebrow"><span />开放的智能层</span><h2>使用你的数据，<br />也可使用你的模型。</h2></div><div className="open-copy"><p>社区版支持配置 OpenAI 兼容 API 地址、密钥和模型。Dashloom Cloud 提供托管 AI 额度、自动分析、报告推送和运行保障。</p><a className="button" href="https://github.com/dashloom-dev/dashloom">在 GitHub 查看 <span>↗</span></a></div></section>
    <footer><a className="brand" href="/zh"><span className="brand-mark" aria-hidden="true"><i /><i /><i /></span><span>Dashloom</span></a><p>让所有产品数据，变成下一步行动。</p><span>© 2026 Dashloom</span></footer>
  </main>;
}
