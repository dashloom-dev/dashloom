...
export default async function DataOverviewPage({ searchParams }: { searchParams: Promise<{ range?: string; sort?: string }> }) {
  ...
  return <div className="app-page data-overview-page">
    <header className="app-page-head"><div><span>{zh ? '按产品查看' : 'BY PRODUCT'}</span><h1>{zh ? '产品数据' : 'Product data'}</h1><p>{zh ? '用相同的时间范围查看每个产品的访客、收入、请求和搜索数据。点击产品可以查看每天的明细。' : 'See visitors, revenue, requests, and search data for every product over the same date range. Open a product to view daily rows.'}</p></div><Link className="app-secondary" href="/dashboard/charts">{zh ? '打开图表' : 'Open charts'} <ArrowRight size={16} /></Link></header>
    ...
    <section className="data-summary-strip"><Summary icon={<Boxes />} label={zh ? '产品' : 'Products'} value={String(cards.length)} note={zh ? '当前列表' : 'in this list'} /><Summary icon={<Database />} label={zh ? '数据源' : 'Sources'} value={String(new Set(points.map((point) => point.source)).size)} note={zh ? '已经导入数据' : 'with imported data'} /><Summary icon={<Layers3 />} label={zh ? '指标' : 'Metrics'} value={String(new Set(points.map((point) => point.metric)).size)} note={zh ? '不同指标名称' : 'different metric names'} /><Summary icon={<CalendarDays />} label={zh ? '最新数据日期' : 'Latest data date'} value={latestDate || '—'} note={latestDate ? (zh ? '来自数据源' : 'from connected sources') : (zh ? '等待首次同步' : 'waiting for first sync')} /></section>
