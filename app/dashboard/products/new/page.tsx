...
export default async function AddProductPage() {
  ...
  return <div className="app-page product-focused-page">
    <header className="app-page-head"><div><span>{zh ? '新产品' : 'NEW PRODUCT'}</span><h1>{zh ? '添加产品' : 'Add product'}</h1><p>{zh ? '填写名称和域名。添加后，你可以为它连接数据源、设置目标并生成报告。' : 'Enter a name and domain. After adding it, you can connect data sources, set goals, and create reports.'}</p></div><Link className="app-secondary" href="/dashboard/products">{zh ? '查看全部产品' : 'View all products'}</Link></header>
