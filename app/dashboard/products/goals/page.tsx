...
export default async function ProductGoalsPage() {
  ...
  return <div className="app-page product-focused-page">
    <header className="app-page-head"><div><span>{zh ? '产品目标' : 'PRODUCT GOALS'}</span><h1>{zh ? '目标' : 'Goals'}</h1><p>{zh ? '为产品选择一个已有指标，填写目标值和截止日期，Dashloom 会在每次同步后更新进度。' : 'Choose an existing metric, enter a target and due date, and Dashloom will update progress after every sync.'}</p></div><Link className="app-secondary" href="/dashboard/products">{zh ? '查看全部产品' : 'View all products'}</Link></header>
