...
export default async function SettingsPage() {
  ...
  const zh = locale === 'zh';
  const community = <div className="tab-section-stack">{workspace && <WorkspaceControls active={{ ...workspace, locale }} />}<section className="settings-form"><h2>{zh ? '社区自部署版本' : 'Community deployment'}</h2><label>{zh ? '版本' : 'Edition'}<input value="Community" readOnly /></label><label>{zh ? 'AI 执行方式' : 'AI execution'}<input value={zh ? '自带 OpenAI 兼容模型服务' : 'Bring your own OpenAI-compatible provider'} readOnly /></label><label>{zh ? '产品' : 'Products'}<input value={entitlements ? `${workspaceProducts.length} · ${zh ? '由自部署环境管理' : 'managed by this deployment'}` : '—'} readOnly /></label><label>{zh ? '计划报告' : 'Report schedules'}<input value={entitlements ? `${schedules.length} · ${zh ? '由自部署环境管理' : 'managed by this deployment'}` : '—'} readOnly /></label><label>{zh ? '自动同步最短间隔' : 'Automatic sync floor'}<input value={entitlements ? (zh ? `由部署方配置 · 系统安全下限 ${entitlements.minimumSyncMinutes} 分钟` : `Deployment managed · ${entitlements.minimumSyncMinutes}-minute safety floor`) : '—'} readOnly /></label><p>{zh ? '凭证、证据、报告和自动化数据均保留在这套自部署环境内。容量取决于自部署基础设施；系统安全护栏不是订阅套餐额度。' : 'Credentials, evidence, reports, and automation stay inside this self-hosted deployment. Capacity depends on your infrastructure; internal safety guards are not subscription allowances.'}</p></section></div>;
  ...
  return <div className="app-page"><header className="app-page-head"><div><span>{zh ? '工作空间控制' : 'WORKSPACE CONTROL'}</span><h1>{zh ? '设置' : 'Settings'}</h1><p>{zh ? '按分类管理自部署工作空间、界面、模型与开发者能力。界面语言由部署配置决定。' : 'Manage your self-hosted workspace, experience, models, and developer capabilities by category. Interface language is deployment-defined.'}</p></div></header><DashboardTabs tabs={[
    { id: 'workspace', label: zh ? '工作空间' : 'Workspace', description: zh ? '基础信息与自部署容量' : 'Defaults and self-hosted capacity', content: community },
