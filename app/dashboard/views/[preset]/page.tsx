...
export default async function IntelligenceViewPage({ params, searchParams }: { params: Promise<{ preset: string }>; searchParams: Promise<{ view?: string }> }) {
  ...
  if (!dates) return <div className="empty-state"><h1>Workspace unavailable</h1><p>The selected workspace could not be loaded.</p></div>;
  ...
  return <div className="app-page"><header className="app-page-head"><div><span>{definition.eyebrow}{savedView ? ` · ${savedView.name}` : ''}</span><h1>{definition.title}</h1><p>{definition.copy}</p></div><a className="app-primary" href={`/dashboard/agent?preset=${definition.agentPreset}`}>Ask {definition.agent}</a></header>
    <section className="view-metric-grid">{metricCards.map(({ metric, currency }) => { const current = totalFor(metric, currency, 'current'); const previous = totalFor(metric, currency, 'previous'); const change = previous ? ((current - previous) / Math.abs(previous)) * 100 : null; return <article key={`${metric}:${currency || ''}`}><span>{humanize(metric)}{currency ? ` · ${currency.toUpperCase()}` : ''}</span><strong>{formatMetric(metric, current, currency)}</strong><small>{change === null ? 'No comparable prior evidence' : `${change >= 0 ? '+' : ''}${change.toFixed(1)}% vs prior 7 days`}</small></article>; })}</section>
    {agentBriefing && sourceRun && <section className="app-panel smart-dashboard-briefing"><div className="panel-title"><div><span>AGENT-GENERATED BRIEFING</span><h2>{agentBriefing.summary}</h2></div><span className="status-pill">Frozen {sourceRun.createdAt.slice(0, 10)}</span></div><div className="smart-dashboard-findings">{agentBriefing.findings.slice(0, 5).map((finding, index) => <article key={`${sourceRun.id}:${index}`} data-severity={finding.severity}><header><strong>{finding.title}</strong><b>{Math.round(finding.confidence * 100)}%</b></header><p>{finding.detail}</p><small>Next: {finding.action}</small><div className="finding-evidence">{finding.evidenceRefs.map((reference) => <code key={reference}>{reference}</code>)}</div></article>)}</div><footer><span>This briefing is a stored Agent conclusion; metric cards above continue to use current workspace evidence.</span><a href={`/dashboard/agent/runs/${sourceRun.id}`}>Inspect frozen evidence →</a></footer></section>}
    <section className="app-panel view-evidence"><div className="panel-title"><div><span>EVIDENCE TABLE</span><h2>Product and metric movement</h2></div><span className="status-pill">{pointRows.length} points · 14d</span></div>{rows.slice(0, 30).map((row) => <article className="evidence-row" key={`${row.productId}:${row.metric}:${row.currency || ''}`}><div><strong>{row.product}</strong><small>{humanize(row.metric)}{row.currency ? ` · ${row.currency.toUpperCase()}` : ''}</small></div><span>{formatMetric(row.metric, row.previous, row.currency)}</span><b>{formatMetric(row.metric, row.current, row.currency)}</b><em>{row.change === null ? '—' : `${row.change >= 0 ? '+' : ''}${row.change.toFixed(1)}%`}</em></article>)}{!rows.length && <div className="panel-empty"><p>This view has no matching evidence yet. Connect a source or import metrics with the names shown above.</p><a href="/dashboard/sources">Connect evidence →</a></div>}</section>
  </div>;
}
...
function formatMetric(metric: string, value: number, currency: string | null = null) {
  if (metric.includes('revenue') || metric === 'mrr' || metric === 'refunds') return currency ? new Intl.NumberFormat('en-US', { style: 'currency', currency: currency.toUpperCase(), maximumFractionDigits: 0 }).format(value) : `${new Intl.NumberFormat('en-US', { maximumFractionDigits: 1 }).format(value)} (currency unset)`;
  ...
  if (metric.endsWith('_duration_ms')) return formatDuration(value);
...
function formatBytes(value: number) { const units = ['B', 'KB', 'MB', 'GB', 'TB', 'PB']; let amount = Math.max(0, value); let unit = 0; while (amount >= 1024 && unit < units.length - 1) { amount /= 1024; unit += 1; } return `${new Intl.NumberFormat('en-US', { maximumFractionDigits: amount < 10 && unit ? 2 : 1 }).format(amount)} ${units[unit]}`; }
function formatDuration(value: number) { const seconds = Math.max(0, value) / 1000; return seconds < 60 ? `${seconds.toFixed(seconds < 10 ? 1 : 0)} s` : `${(seconds / 60).toFixed(seconds < 600 ? 1 : 0)} min`; }
