...
export default async function SourcesPage({ searchParams }: { searchParams: Promise<{ google?: string }> }) {
  ...
  const growth = <DashboardTabs initialTab="google" tabs={[
    ...
    { id: 'stripe', label: 'Stripe', description: zh ? '收入与订阅' : 'Revenue & subscriptions', content: <div className="tab-section-stack">{title('STRIPE', zh ? '连接收入证据' : 'Connect commercial evidence')}<StripeForm products={productRows} /></div> },
    ...
    { id: 'creem', label: 'Creem', description: zh ? '交易与退款' : 'Payments & refunds', content: <div className="tab-section-stack">{title('CREEM', zh ? '连接 AI SaaS 支付证据' : 'Connect AI SaaS payment evidence')}<CreemForm products={productRows} /></div> },
