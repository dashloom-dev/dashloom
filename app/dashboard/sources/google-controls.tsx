...
export function GoogleControls({ accounts, resources, products, initialStatus, zh }: { accounts: Account[]; resources: Resource[]; products: Array<{ id: string; name: string }>; initialStatus?: string; zh: boolean }) {
  ...
  async function sync() {
    setPending(true);
    setMessage('Syncing GA4 and Search Console…');
    try {
      const response = await fetch('/api/sync/google', { method: 'POST' });
      const result = await response.json() as { error?: string; written?: number; errors?: string[] };
      const errors = result.errors?.join('; ');
      setMessage(result.error || (!response.ok
        ? `Google sync failed (HTTP ${response.status}). Try again.`
        : `${result.written || 0} Google metric points synchronized.${errors ? ` Sync incomplete: ${errors} Retry, or authorize Google again if access has expired.` : ''}`));
      router.refresh();
    } catch {
      setMessage('Could not confirm the Google sync result. Refresh and retry.');
    } finally {
      setPending(false);
    }
  }
  ...
  return <section className="google-controls"><header><div><span>GOOGLE OAUTH</span><h3>{accounts.length ? `${accounts.length} connected Google account${accounts.length === 1 ? '' : 's'}` : 'Connect Analytics and Search Console'}</h3></div><a className="app-primary" href="/api/connectors/google/start">Connect Google account</a></header>
    ...
    {message && <div><p className="form-message" role="status">{message}</p>{accounts.length > 0 && <button className="app-secondary" type="button" disabled={pending} onClick={sync}>{zh ? '重试同步' : 'Retry sync'}</button>}</div>}
  </section>;
}
