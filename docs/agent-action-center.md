...
Agent output may reference only product IDs and evidence IDs present in the saved data snapshot for that run. Unknown products, invented citations, unlabeled causal claims, and undisclosed truncated evidence are rejected before findings or actions are stored.
