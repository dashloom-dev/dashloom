...
使用 `en` 选择英文，使用 `zh-CN` 选择简体中文。它决定整个 Community 部署的登录、找回邮件、导航和产品页面语言。Community 不提供本地化 Dashboard 路由或应用内语言切换；如需修改部署语言，请调整该变量并重新构建。
...
2. 确认 Dashboard 导航和产品页面使用该语言，且工作空间之间仍保持隔离。
3. 添加一个产品并完成一次连接器同步。
