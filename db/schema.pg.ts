...
export const workspaceMembers = pgTable('workspace_members', {
  ...
  joinedAt: text('joined_at').notNull().default(sql`CURRENT_TIMESTAMP::text`),
...
export const connectorAccounts = pgTable('connector_accounts', {
  id: text('id').primaryKey(),
  workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
  provider: text('provider', { enum: ['cloudflare', 'cloudflare_pages', 'cloudflare_queues', 'google', 'bing', 'd1', 'stripe', 'lemonsqueezy', 'creem', 'polar', 'paddle', 'supabase', 'github', 'vercel', 'custom'] }).notNull(),
...
export const productConnectorMappings = pgTable('product_connector_mappings', {
  id: text('id').primaryKey(),
  workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
  productId: text('product_id').notNull().references(() => products.id, { onDelete: 'cascade' }),
  connectorAccountId: text('connector_account_id').notNull().references(() => connectorAccounts.id, { onDelete: 'cascade' }),
  source: text('source', { enum: ['cloudflare', 'cloudflare_r2', 'cloudflare_pages', 'cloudflare_queues', 'ga4', 'gsc', 'bing', 'd1', 'stripe', 'lemonsqueezy', 'creem', 'polar', 'paddle', 'supabase', 'github', 'vercel', 'business', 'custom'] }).notNull(),
...
export const connectorResources = pgTable('connector_resources', {
  ...
  workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
  connectorAccountId: text('connector_account_id').notNull().references(() => connectorAccounts.id, { onDelete: 'cascade' }),
  type: text('type', { enum: ['ga4', 'gsc', 'bing_site', 'worker', 'database', 'custom'] }).notNull(),
  resourceId: text('resource_id').notNull(),
  displayName: text('display_name').notNull(),
  domainsJson: text('domains_json').notNull().default('[]'),
  permissionLevel: text('permission_level'),
  discoveredAt: text('discovered_at').notNull().default(sql`CURRENT_TIMESTAMP::text`),
...
export const metricPoints = pgTable('metric_points', {
  ...
  collectedAt: text('collected_at').notNull().default(sql`CURRENT_TIMESTAMP::text`),
...
export const calculatedMetricDefinitions = pgTable('calculated_metric_definitions', {
  id: text('id').primaryKey(),
  workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
  name: text('name').notNull(),
  metric: text('metric').notNull(),
  ...
  constantValue: doublePrecision('constant_value'),
...
export const competitorMetricPoints = pgTable('competitor_metric_points', {
  id: serial('id').primaryKey(),
  workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
...
export const agentConversations = pgTable('agent_conversations', {
  ...
  createdByUserId: text('created_by_user_id').notNull(),
  lastMessageAt: text('last_message_at').notNull().default(sql`CURRENT_TIMESTAMP::text`),
  ...timestamps,
...
export const agentExecutiveBriefs = pgTable('agent_executive_briefs', {
  id: text('id').primaryKey(),
  workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
  scopeMode: text('scope_mode', { enum: ['workspace', 'product'] }).notNull().default('workspace'),
  productId: text('product_id').references(() => products.id, { onDelete: 'set null' }),
...
export const agentActionOccurrences = pgTable('agent_action_occurrences', {
  id: text('id').primaryKey(),
  workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
...
export const agentGrowthMissions = pgTable('agent_growth_missions', {
  id: text('id').primaryKey(),
  workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
...
export const agentComparisonRuns = pgTable('agent_comparison_runs', {
  id: text('id').primaryKey(),
  workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
  agentPreset: text('agent_preset', { enum: ['portfolio_analyst', 'revenue_analyst', 'seo_growth_analyst', 'operations_analyst', 'client_reporting_analyst'] }).notNull(),
  question: text('question').notNull(),
...
export const agentComparisonResults = pgTable('agent_comparison_results', {
  id: text('id').primaryKey(),
  workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
...
export const ingestionApiKeys = pgTable('ingestion_api_keys', {
  id: text('id').primaryKey(),
  workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
  productId: text('product_id').references(() => products.id, { onDelete: 'cascade' }),
  name: text('name').notNull(),
...
export const syncSchedules = pgTable('sync_schedules', {
  id: text('id').primaryKey(),
  workspaceId: text('workspace_id').notNull().references(() => workspaces.id, { onDelete: 'cascade' }),
  source: text('source', { enum: ['cloudflare', 'cloudflare_r2', 'cloudflare_pages', 'cloudflare_queues', 'google', 'bing', 'd1', 'stripe', 'lemonsqueezy', 'creem', 'polar', 'paddle', 'supabase', 'github', 'vercel', 'custom'] }).notNull(),
