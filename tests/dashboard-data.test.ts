...
test('dashboard loading excludes only bounded GSC breakdown rows and retains GA4 page views', () => {
  ...
  const policy = readFileSync(new URL('../lib/dashboard-summary.ts', import.meta.url), 'utf8');
  assert.match(policy, /dashboardBreakdownMetrics = \['query_clicks', 'query_impressions', 'query_position', 'page_clicks', 'page_impressions', 'page_position'\]/);
  assert.match(source, /import \{ dashboardBreakdownMetrics \} from '\.\/dashboard-summary'/);
